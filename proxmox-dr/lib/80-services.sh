...
deploy_control_vm_stack() {
    ...
    # Copy .env file from deployment package to Control VM
    local local_env_file="${SCRIPT_DIR}/control-vm-config/.env"
    if [[ -f "$local_env_file" ]]; then
        ...
        log_warn "No .env file found in deployment package - setup script will fail"
    fi
